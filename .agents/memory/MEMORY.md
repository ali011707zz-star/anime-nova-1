...
- [Device-link Supabase fallback](device-link-supabase-fallback.md) — production Supabase may lack device-link/push tables; VPS PostgreSQL fallback must handle 4xx schema responses consistently.
...
- [Kawaii CDN rotation](kawaii-api.md) — `/api/miruro` currently returns unsigned HLS on `cdn.imgnex.top`; the server trust filter must account for this verified provider CDN.
...
- [Foupix mobile playback](foupix-mobile-playback.md) — direct Foupix MP4 can fail Android TLS and the first CDN request can transiently 403; use the public progressive proxy with retries.
