...
- **Arabic translation**: the API uses the Node `node-google-translator` client first, then the Python `googletrans` client as an independent fallback, followed by the existing direct Google/LibreTranslate/MyMemory fallbacks. Translation tokens are protected so URLs, handles, and episode identifiers remain intact.
