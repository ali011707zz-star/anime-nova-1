...
import { db, watchHistory, favorites, watchProgress } from "@workspace/db";
...
router.post("/user/history", async (req: Request, res: Response) => {
  ...
  try {
    ...
    /* Delete existing entry for same (userId, animeId, ep, season) then insert fresh */
    await db
      .delete(watchHistory)
      .where(
        and(
          ...
          eq(watchHistory.seasonNumber, Number(seasonNumber) || 1),
        )
      );
...
router.delete("/user/history/:id", async (req: Request, res: Response) => {
  ...
  try {
    await db
      .delete(watchHistory)
      .where(and(eq(watchHistory.id, req.params.id), eq(watchHistory.userId, userId)));
...
/* GET /api/user/favorites/ids — only the anime IDs */
router.get("/user/favorites/ids", async (req: Request, res: Response) => {
  ...
  try {
    const rows = await db
      .select({ animeId: favorites.animeId })
      .from(favorites)
      .where(eq(favorites.userId, userId));

    return res.json({ ids: rows.map((r: { animeId: number }) => r.animeId) });
  } catch (err) {
...
router.post("/user/favorites", async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(201).json({ entry: row || null, already: !row });
  } catch (err) {
    ...
    await db
      ...
      .where(
        and(
          ...
          eq(favorites.animeId, Number(req.params.animeId))
        )
      );
    return res.json({ ok: true });
  } catch (err) {
...
/* GET /api/user/progress/all — all latest progress entries */
...
/* GET /api/user/progress/:animeId/:ep?season=1 */
...
/* GET /api/user/continue-watching — progress > 30s, not finished */
router.get("/user/continue-watching", async (req: Request, res: Response) => {
  ...
  try {
    ...
    /* Filter: must have > 30s progress and not be >95% done */
    const active = rows.filter((r: typeof rows[number]) =>
      r.progressSeconds > 30 &&
      (r.durationSeconds === 0 || r.progressSeconds < (r.durationSeconds ?? 0) * 0.95)
    );
