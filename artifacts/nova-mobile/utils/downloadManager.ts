...
async function saveCompleted(entry: RuntimeDownload): Promise<void> {
  ...
  /* KW is converted by the VPS download endpoint with the Arabic subtitles
     burned into the MP4. Do not create a misleading sidecar VTT for it;
     other sources keep the existing sidecar behavior. */
  if (entry.params.subtitleUrl && entry.params.site !== "kawaii") {
