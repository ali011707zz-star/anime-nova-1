...
export default function AwDubbedDetailScreen() {
  ...
  const { key: keyParam, title: titleParam, titleAr: titleArParam, seasons: seasonsParam, poster: posterParam } =
    useLocalSearchParams<{ key: string; title: string; titleAr: string; seasons: string; poster: string; }>();

  const title    = titleParam   || decodeURIComponent(keyParam || "");
  const titleAr  = titleArParam || "";
  const poster   = posterParam  ? decodeURIComponent(posterParam) : null;
  ...
  const renderEp = useCallback(({ item: ep }: { item: Episode }) => (
    ...
    >
      ...
      <View style={styles.epInfo}>
        ...
        <Text style={styles.epSub}>{curSeason?.label || "الموسم 1"}</Text>
      </View>
            ...
            <View style={styles.heroRow}>
              ...
              <View style={styles.heroInfo}>
                ...
                {title !== titleAr && titleAr ? (
                  <Text style={styles.heroSub}>{title}</Text>
                ) : null}
                <View style={styles.heroBadge}>
                  <Ionicons name="tv-outline" size={11} color="#10B981" />
                  <Text style={styles.heroBadgeText}>أنيميشن مدبلج</Text>
                </View>
              </View>
            </View>
              ...
              <View style={styles.seasonsWrap}>
                ...
                >
                  {seasons.map((s, i) => (
                    <Pressable
                      key={s.animeId}
                      onPress={() => setSelSeason(i)}
                      style={[styles.seasonTab, selSeason === i && styles.seasonTabActive]}
                    >
                      <Text style={[styles.seasonTabText, selSeason === i && styles.seasonTabTextActive]}>
                        {s.label}
                      </Text>
                    </Pressable>
                  ))}
