...
public class InterpolateOnScrollPositionChangeHelper {
    ...
    public void updateInterpolationForScreenPosition() {
        ...
        if (scrollView.getChildCount() != 0) {
            ...
            if (top < 0) {
                this.materialShapeDrawable.setInterpolation(Math.max(StaticLayoutBuilderCompat.DEFAULT_LINE_SPACING_ADD, Math.min(1.0f, (top / height) + 1.0f)));
                this.shapedView.invalidate();
                return;
            }
            if (top + height > height2) {
                this.materialShapeDrawable.setInterpolation(Math.max(StaticLayoutBuilderCompat.DEFAULT_LINE_SPACING_ADD, Math.min(1.0f, 1.0f - ((r0 - height2) / height))));
                this.shapedView.invalidate();
                return;
            } else {
