 ...
 * AuthCallback.tsx
 * ─────────────────────────────────────────────────────
 * صفحة /auth/callback — تستقبل redirect من Supabase بعد Google OAuth
 * Supabase يُعيد: /auth/callback#access_token=...&token_type=bearer
 * نستخرج access_token من الـ hash ونُرسله للـ backend
 */
...
export default function AuthCallback() {
  ...
  useEffect(() => {
    ...
    /* أحياناً Supabase يضع الـ params في query string بدل hash */
    const qParams     = new URLSearchParams(window.location.search);
    const qError      = qParams.get("error");
    const qErrorDesc  = qParams.get("error_description");

    if (errorParam || qError) {
      setErrorMsg(errorDesc || qErrorDesc || errorParam || qError || "خطأ غير معروف");
      setStatus("error");
    ...
    if (!accessToken) {
      setErrorMsg("لم يصل access_token من Supabase");
      setStatus("error");
      setTimeout(() => navigate("/auth"), 3000);
      return;
    }
      ...
      {status === "loading" ? (
        <>
          ...
          <div className="text-center">
            ...
            <p className="text-white/35 text-sm font-['Cairo'] mt-1">نتحقق من حساب Google</p>
          </div>
        </>
      ) : (
        <>
